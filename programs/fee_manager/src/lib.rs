...
pub mod fee_manager {
    ...
    pub fn initialize(
        ...
        fee_manager.gov = gov;
        fee_manager.c3caller_proxy = c3caller_proxy;
        fee_manager.dapp_id = dapp_id;
        fee_manager.tx_senders.push(tx_sender);
        fee_manager.from_chain_pay = 1;
        fee_manager.to_chain_pay = 2;
        fee_manager.bump = *ctx.bumps.get("fee_manager").unwrap();
        Ok(())
    }
